...
import datetime
import math
from cet.plots import plot_ldaprototype_wordcloud


def create_topic_wordcloud_reports(
    lda_model: LDAPrototype,
    n_cols: int = 2,
    topic_names: Optional[list[str]] = None,
    title: Optional[str] = None,
) -> Tuple[Figure, Axes]:
    """Generate wordcloud report per lda

    Args:
        lda_model (LDAPrototype): Prototype LDA model from ttta library
        n_cols (int, optional): Number of columns for the subplot grid. Defaults to 2.
        num_topwords (int, optional): Number of top words to display in the wordcloud. Defaults to 20.

    Returns:
        Tuple[Figure, Axes]: Figure and Axes objects containing the wordclouds
    """
    k = lda_model._K
    ...
    for topic in range(k):
        ...
        row = topic // n_cols
        col = topic % n_cols
        wordcloud = plot_ldaprototype_wordcloud(lda_model, topic=topic)
        ax[row, col].imshow(wordcloud, interpolation="antialiased")
        ax[row, col].axis("off")
        ## top words,
        if topic_names is not None:
            ax[row, col].set_title(
                f"Topic {topic} - {topic_names[topic]}",
                fontsize=8,
                wrap=True,
            )
        else:
            # just write the topic number
            ax[row, col].set_title(
                f"Topic {topic}",
                fontsize=8,
                wrap=True,
    ...
    if title is not None:
        ...
        fig.suptitle(f"LDA Topic WordClouds K={k}", fontsize=16)
    fig.tight_layout(h_pad=1.10, w_pad=1.10)
